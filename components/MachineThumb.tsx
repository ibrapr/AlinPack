...
const palettes: Record<MachineCategory, { from: string; to: string; accent: string }> = {
  filling: { from: '#e7f0fa', to: '#ffffff', accent: '#0B5DAA' },
  sealing: { from: '#e0e7ff', to: '#eef2ff', accent: '#2563eb' },
  capping: { from: '#eaf4ff', to: '#ffffff', accent: '#2F80D8' },
...
export default function MachineThumb({ category, image, alt }: MachineThumbProps) {
  ...
  return (
    ...
    >
      ...
      {/* Machine body */}
      <rect x="90" y="70" width="140" height="120" rx="8" fill="#0B1F3A" />
      ...
      {/* Hopper */}
      <path d="M120 70 L200 70 L188 36 L132 36 Z" fill="#12345A" />
      <ellipse cx="160" cy="36" rx="28" ry="4" fill="#0B1F3A" />

      {/* HMI */}
      <rect x="104" y="86" width="46" height="34" rx="3" fill="#0B1F3A" />
      ...
      {/* Right panel */}
      <rect x="160" y="86" width="60" height="34" rx="3" fill="#0B1F3A" />
      ...
      <rect x="138" y="130" width="44" height="20" rx="2" fill="#0B1F3A" />
      <rect x="146" y="150" width="28" height="32" rx="2" fill="#12345A" />
      <rect x="155" y="182" width="10" height="6" fill="#456985" />

      {/* Side knobs */}
      <circle cx="102" cy="148" r="7" fill="#0B1F3A" />
      <circle cx="102" cy="148" r="3" fill={palette.accent} />
      <circle cx="218" cy="148" r="7" fill="#0B1F3A" />
      <circle cx="218" cy="148" r="3" fill="#22c55e" />

      {/* Conveyor */}
      <rect x="40" y="190" width="240" height="14" rx="3" fill="#274767" />
      {Array.from({ length: 10 }).map((_, i) => (
        <rect key={i} x={50 + i * 22} y={192} width={14} height={10} rx={1} fill="#456985" />
      ))}
      ...
      {[60, 100, 220, 260].map((x, i) => (
        <g key={i}>
          ...
          <rect x={x - 3} y={164} width={6} height={6} rx={1} fill="#0B1F3A" />
        </g>
      ))}

      {/* Brand mini */}
      <rect x="124" y="166" width="72" height="10" rx="2" fill="#0B1F3A" />
      <text
        x="160"
        y="174"
